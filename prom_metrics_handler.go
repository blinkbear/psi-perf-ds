...
func _queryPsi(podPidPath map[string]map[string]string) {
	for podName, containerPath := range podPidPath {
		...
		for container, path := range containerPath {
			...
			cpuPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "10s"}).Set(cpuSome10)
			cpuPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "60s"}).Set(cpuSome60)
			cpuPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "300s"}).Set(cpuSome300)
			cpuPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "total"}).Set(cpuSomeTotal)
			...
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "10s"}).Set(memSome10)
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "60s"}).Set(memSome60)
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "300s"}).Set(memSome300)
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "total"}).Set(memSomeTotal)
			...
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "10s"}).Set(memFull10)
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "60s"}).Set(memFull60)
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "300s"}).Set(memFull300)
			memPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "total"}).Set(memFullTotal)
			...
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "10s"}).Set(ioSome10)
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "60s"}).Set(ioSome60)
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "300s"}).Set(ioSome300)
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "some", "window": "total"}).Set(ioSomeTotal)
			...
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "10s"}).Set(ioFull10)
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "60s"}).Set(ioFull60)
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "300s"}).Set(ioFull300)
			ioPsiGauge.With(prometheus.Labels{"pod_name": podName, "container_name": container, "type": "full", "window": "total"}).Set(ioFullTotal)
		}
	}
